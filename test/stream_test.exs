defmodule StreamTest do
  ...
  test "stream a closed query is ok ( parse again )", context do
    # this one causes an error log on test stdout ?
    assert {:ok, _} = transaction( fn(conn) ->
  ...
  test "raise on unprepared query", context do
	...
	test "stream struct interpolates to its statement" do
    q = %Postgrex.Query{name: "ENOENT", statement: "SELECT 42"}
    assert "#{ %Postgrex.Stream{query: q} }" == "SELECT 42"
  end

  test "connection_id is pg_backend_pid()", context do
    assert [ %Postgrex.Result{ connection_id: backend_pid, rows: [[ backend_pid ]] } ] =
      stream( prepare( "", "SELECT pg_backend_pid()" ), [] ) |> Enum.take( 1 )
  end


  # streams each int i in range as [ i ] using a name as statement name
  # range( pid, "whatever", 1, 2 ) |> Enum.flat_map( &(&1) )
  # => [ 1, 2 ]
  #
  ...
  # nest two ranges, first query has name1, second has name2
  #
